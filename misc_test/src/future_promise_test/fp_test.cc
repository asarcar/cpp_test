...
class FPTest {
 public:
  explicit FPTest(int argc, char **argv); 
  ~FPTest() = default;

  FPTest(const FPTest &at) = delete;
  FPTest(FPTest &&at) = delete;
  FPTest& operator=(const FPTest &at) = delete;
  FPTest& operator=(FPTest &&at) = delete;
...
FPTest:: FPTest(int argc, char **argv) {
  // Parse Arguments...
  if (argc == 1) {
    _siz = DEF_SIZ;
  } else {
    ...
    if (argc >= 3) {
      if (std::stoi(std::string(argv[2])) == 1)
        _excep = true;
    }
    if ((s < 0) || (argc > 3)) {
      std::ostringstream oss;
      oss << "Usage: " << argv[0] << " [arr_siz [throw_exception_flag]]" << std::endl;
      throw oss.str();
    }
    _siz = s;
  }
...
void FPTest::spawn_threads(void) {
  ...
  for (uint32_t j=0; j<_range; ++j) {
    ...
    _future_sums.push_back(pr.get_future());
    
    // Spawn the Thread
    std::thread t([this, j](std::promise<uint32_t>&& prom) {
        try {
          uint32_t val = std::accumulate(this->_vals.begin()+
                                         ...
                                         this->_lims.at(j+1), 
                                         0);
          if ((this->_excep == true) && (j > _range/2)) {
            std::ostringstream oss;
            oss << "Exception thrown by ThID: " << std::this_thread::get_id() 
                << " ignoring value " << val 
                << " for range ["
...
uint32_t FPTest::compute_sum(void) {
  _sum=0;
  for (auto &fut:_future_sums) {
    try {
      ...
      std::cerr << "Main ThID " << std::this_thread::get_id() << ": " << str
                << ": ignoring range & value for that thread" << std::endl;
    }
  }
