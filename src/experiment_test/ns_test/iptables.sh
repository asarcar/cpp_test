...
# NAT 
# CHAIN RULES:
#   PREROUTING: chain apply to inbound pkts (pkts arriving at the GW from any dir). 
#   OUTPUT: chain apply to locally generated pkts (generated on the GW itself). 
#   POSTROUTING: chain apply to outbound pkts (pkts leaving the GW in any dir).
# TARGETS:
#   MASQUERADE: Used in POSTROUTING. ONE DHCP based public side IP address.
#   SNAT: Used in POSTROUTING. Pool of static public IP address.
#         Typical: Address conns initiated from more trusted clients behind NAT.
#   DNAT: Used in PREROUTING/OUTPUT. Translate from less->more trusted NAT.
#         Unusual: Address conns destined to more trusted servers behind NAT.
#   NETMAP: Static 1<->1 mapping between 2 nwk blocks of equal size.
#   REDIRECT: Used in PREPROUTING/OUTPUT. Translate the dst addr/port of any packet 
#             to the addr of IF where it arrived. Facilitates use of transparent 
#             proxies e.g. browsers automatically redirected to a proxy server. 
#   
...
########################################################################################
# UFW: Ubuntu Firewall
# ufw status; ufw enable|disable; ufw default allow|deny
# ufw allow|deny from ip to port; ufw delete allow|deny
########################################################################################
